...
void run_flash_fwd(Flash_fwd_params& params, cudaStream_t stream) {
  ...
  typename CollectiveEpilogue::Params epilogue_params =
      CollectiveEpilogue::to_underlying_arguments({
          ...
          seqlen_traits_q.get_lse_gmem_layout(
              params.seqlen_q, params.h, params.b) // layout_LSE
      });
  ...
  if constexpr (cutlass::sizeof_bits_v<Element> == 8)
    kernel = (void*)flash::
        compute_attn_ws_fp8<Kernel_traits, Is_causal, Scheduler, Seqlen_traits>;
  else
    kernel = (void*)flash::
        compute_attn_ws<Kernel_traits, Is_causal, Scheduler, Seqlen_traits>;
...
void run_mha_fwd_hdim64_fp8(Flash_fwd_params& params, cudaStream_t stream) {
  ...
  if (params.is_causal) {
    ...
    BOOL_SWITCH(
        cutlass::ceil_div(params.seqlen_q, kBlockM) % 2 == 0, UseCluster, [&] {
          run_flash_fwd<
              Flash_fwd_kernel_traits_fp8<
                  Headdim,
                  kBlockM,
