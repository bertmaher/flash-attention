  ...
  int get_n_block_max(
    ...
    if constexpr (Is_causal) {
      n_block_max = std::min(
          n_block_max,
          cute::ceil_div(
              (m_block + 1) * kBlockM + seqlen_k - seqlen_q, kBlockN));
    }
    return n_block_max;
  }
...
#pragma unroll
      ...
      cute::copy(
          softmax.template max</*Is_first=*/false, /*Check_inf=*/true>(tSrS),
          scores_scale);
...
#pragma unroll 1
    ...
    flash::gemm</*zero_init=*/false, /*wg_wait=*/-1>(
        tiled_mma1, tOrP, tOrV(_, _, _, smem_pipe_read_v.index()), tOrO);
    cute::copy(
        softmax.template finalize</*Check_inf=*/Is_causal>(tSrS), scores_scale);
    warpgroup_wait<0>();
...
#pragma unroll
      ...
      for (int iter = 0; iter < extra_iterations && n_block >= 0;
        ...
        cute::copy(
            softmax.template max</*Is_first=*/false>(tSrS), scores_scale);
        softmax.rescale_o(tOrO, scores_scale);
        softmax.template online_softmax</*Is_first=*/false>(tSrS);
        Tensor tOrP = make_tensor(
            convert_type<Element>(tSrS).data(),
    ...
    cutlass::arch::NamedBarrier::arrive(
        ...
        static_cast<int>(FwdNamedBarriers::QueryEmpty) /*id*/);
    cute::copy(
        softmax.template finalize</*Check_inf=*/Is_causal>(
            tSrS, shared_storage.descale_v),
        scores_scale);
    softmax.rescale_o(tOrO, scores_scale);
    return;
  }
