...
class SeqLenTraits {
 public:
  ...
  CUTLASS_DEVICE auto get_local_tile_tensor(
      ...
      bool padded = false) const {
    auto g_tensor =
        local_tile(m_tensor(_, _, bidh, bidb), tile_shape, make_coord(_, _0{}));
    return g_tensor;
  }
  ...
  CUTLASS_DEVICE auto get_lse_local_tile_tensor(
      ...
      bool padded = false) const {
    auto g_tensor =
        local_tile(m_tensor(bidb, bidh, _), tile_shape, make_coord(_));
    return g_tensor;
  }
};
...
CUTLASS_HOST_DEVICE auto VarSeqLenTraits::get_lse_gmem_layout(
  ...
  return make_layout(
      ...
      make_stride(
          int64_t(sum_s + (padded ? kMaxTileSize * b : 0)), cute::_1()));
}
