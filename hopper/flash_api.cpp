...
std::vector<at::Tensor> mha_fwd(
  ...
  TORCH_CHECK(
      head_size_og == 64 || head_size_og == 128 || head_size_og == 256,
  ...
  if (out_.has_value()) {
    ...
    // inputs");
    TORCH_CHECK(
        q_dtype == at::ScalarType::Float8_e4m3fn ? (out.dtype() == at::kHalf)
                                                 : (out.dtype() == q_dtype),
  ...
  auto tile_count_semaphore = is_causal
...
std::vector<at::Tensor> mha_varlen_fwd(
  ...
  if (seqused_k.has_value()) {
    ...
    TORCH_CHECK(
        seqused_k_.dtype() == torch::kInt32, "seqused_k must have dtype int32");
...
std::vector<at::Tensor> mha_bwd(
  ...
  TORCH_CHECK(
      is_sm9x, "FlashAttentionHopper only supports Hopper GPUs or newer.");
