...
struct CollectiveMainloopBwd {
  ...
  static_assert(
      !(SdP_swapAB && dKV_swapAB)); // If SdP_swapAB, then we don't swap for dKV
  ...
  static constexpr bool Mma_dQ_is_RS = AtomLayoutMSdP == 2 &&
      AtomLayoutMdQ == 2 && !SdP_swapAB &&
  ...
  using SmemLayoutLSEMma = cute::Layout<
  ...
  using SmemLayoutPt = decltype(cute::composition(
      SmemLayoutP{},
      make_layout(
          make_shape(get<1>(TileShape_MNK{}), get<0>(TileShape_MNK{})),
  ...
  using TileShape_dQaccum = cute::
      Shape<Int<(kBlockM * kHeadDim) / ElemsPerRowTMA>, Int<ElemsPerRowTMA>>;
  // using TileShape_dQaccum = cute::Shape<Int<kBlockM>, Int<kHeadDim>>;
  using SmemLayoutdQaccumTMA = decltype(tile_to_shape(
      GMMA::Layout_K_SW128_Atom<ElementAccum>{},
      TileShape_dQaccum{}));
  ...
  static Params to_underlying_arguments(Arguments const& args) {
    ...
    Tensor mdQaccum = make_tensor(
        make_gmem_ptr(args.ptr_dQaccum),
        args.shape_dQaccum,
        args.stride_dQaccum);
    ...
    Tensor mLSE = make_tensor(
        make_gmem_ptr(args.ptr_LSE_log2), args.shape_LSE, args.stride_LSE_log2);
    ...
    Tensor mdPsum = make_tensor(
        make_gmem_ptr(args.ptr_dPsum), args.shape_LSE, args.stride_dPsum);
  ...
  CUTLASS_DEVICE void load(
    ...
    Tensor sLSE = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_lse.data()),
        SmemLayoutLSE{});
    Tensor sdPsum = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_dpsum.data()),
        SmemLayoutLSE{});
    ...
    Tensor gQ = local_tile(
        domain_offset(make_coord(offset_q, _0{}), mQ),
        select<0, 2>(TileShape_MNK{}),
    ...
    Tensor gLSE = local_tile(
        domain_offset(make_coord(offset_padded), mLSE),
        select<0>(TileShape_MNK{}),
        make_coord(_)); // (M, _)
    Tensor gdPsum = local_tile(
        domain_offset(make_coord(offset_padded), mdPsum),
        select<0>(TileShape_MNK{}),
        make_coord(_)); // (M, _)
    ...
    auto [tQgQ, tQsQ] = tma_partition(
        ...
        Layout<ClusterShape>{},
        group_modes<0, 2>(sQ),
        group_modes<0, 2>(gQ)); // (TMA, k), (TMA, PIPE)
    auto [tdOgdO, tdOsdO] = tma_partition(
        params.tma_load_dO,
        block_rank_in_cluster,
        Layout<ClusterShape>{},
        group_modes<0, 2>(sdO),
    ...
    auto [tLSEgLSE, tLSEsLSE] = tma_partition(
        params.tma_load_LSE,
        _0{},
        Layout<_1>{},
        sLSE,
        gLSE); // (TMA, k), (TMA, PIPE)
    auto [tLSEgdPsum, tLSEsdPsum] = tma_partition(
        params.tma_load_dPsum,
        _0{},
        Layout<_1>{},
        sdPsum,
    ...
    if (lane_predicate) {
      ...
      copy(
          params.tma_load_LSE.with(
              *pipeline_q.producer_get_barrier(smem_pipe_write), 0),
          tLSEgLSE(_, m_block),
          tLSEsLSE(_, smem_pipe_write.index()));
#pragma unroll 2
        ...
        copy(
            params.tma_load_dO.with(
                *pipeline_do.producer_get_barrier(smem_pipe_write),
                mcast_mask_qdo),
            tdOgdO(_, m_block),
            tdOsdO(_, smem_pipe_write.index()));
        copy(
            params.tma_load_dPsum.with(
                *pipeline_do.producer_get_barrier(smem_pipe_write), 0),
            tLSEgdPsum(_, m_block),
            tLSEsdPsum(_, smem_pipe_write.index()));
        ++smem_pipe_write;
        pipeline_q.producer_acquire(smem_pipe_write);
        copy(
            params.tma_load_Q.with(
  ...
  CUTLASS_DEVICE void store_dq(
    ...
    Tensor sdQ = make_tensor(
        make_smem_ptr(shared_storage.mainloop.smem_dqacc.data()),
        SmemLayoutdQaccumTMA{});
    ...
    int const offset_padded = !is_varlen_q
        ? 0
    ...
    auto compute_dQ = [&]() {
      ...
      if constexpr (!dQ_swapAB) {
        ...
        flash::gemm</*zero_init=*/true, /*wg_wait=*/1>(
            tiled_mma_dQ,
            tdQrdS(_, _, _, smem_pipe_read.index()),
            tdQrK,
            tdQrdQ);
      } else {
    ...
    if constexpr (Is_causal) {
      ...
      for (; m_block < std::min(m_block_max, m_block_min + n_masking_steps);
        ...
        flash::gemm</*zero_init=*/true, /*wg_wait=*/-1>(
            tiled_mma_SdP,
            tdPrV,
            tdPrdO(_, _, _, smem_pipe_read.index()),
            tdPrdP);
        warpgroup_wait<1>();
...
#pragma unroll
        for (int i = 0; i < size(tSrS); ++i) {
          if (int(get<0>(taccScS(i))) >=
              std::min(
                  int(get<1>(taccScS(i))) + causal_row_offset,
...
#pragma unroll
        ...
        flash::gemm</*zero_init=*/false, /*wg_wait=*/-1>(
            tiled_mma_dKV,
            tdVrP,
            tdVrdO(_, _, _, smem_pipe_read.index()),
            tdVrdV);
...
#pragma unroll
      ...
      Tensor tdSadS =
          smem_thr_copy_PdS.retile_S(rdS); // ((Atom,AtomNum), MMA_N, MMA_N)
      cute::copy(
          smem_tiled_copy_PdS, tdSadS, tdSsdS(_, _, _, smem_pipe_read.index()));
