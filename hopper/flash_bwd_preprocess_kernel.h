...
class FlashAttnBwdPreprocess {
 public:
  ...
  static_assert(
      get<1>(TileShape_MK{}) % kGmemElemsPerLoadAccum == 0,
  ...
  using GmemLayoutAtomAccum = Layout<
      Shape<
          Int<MaxThreadsPerBlock / kGmemThreadsPerRowAccum>,
  ...
  void operator()(Params const& params, [[maybe_unused]] char* smem_buf) {
    ...
    Tensor mO = make_tensor(
        ...
        _, _, bidh, !is_varlen ? bidb : 0);
    Tensor gO = local_tile(
        cute::domain_offset(make_coord(offset_o, _0{}), mO),
        TileShape_MK{},
    ...
    Tensor gdO = local_tile(
        cute::domain_offset(make_coord(offset_o, _0{}), mdO),
        TileShape_MK{},
        make_coord(m_block, _0{})); // (M, K)
    ...
    Tensor mLSE = make_tensor(
        ...
        _, bidh, !is_varlen ? bidb : 0);
    Tensor gLSE = local_tile(
        cute::domain_offset(make_coord(offset_o), mLSE),
        Shape<Int<kBlockM>>{},
        make_coord(m_block));
...
#pragma unroll
    ...
    flash::
        copy</*Is_even_MN=*/false, /*Is_even_K=*/false, /*Clear_OOB_MN=*/true>(
            gmem_tiled_copy_O,
            tOgO,
            tOrO,
            tOcO,
...
#pragma unroll
    ...
    Tensor gdPsum = local_tile(
        cute::domain_offset(make_coord(offset_padded), mdPsum),
        Shape<Int<kBlockM>>{},
        make_coord(m_block));
...
#pragma unroll
    ...
    Tensor gLSElog2 = local_tile(
        cute::domain_offset(make_coord(offset_padded), mLSElog2),
        Shape<Int<kBlockM>>{},
        make_coord(m_block));
