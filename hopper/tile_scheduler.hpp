...
class DynamicPersistentTileScheduler {
 ...
 public:
  ...
  get_next_work(Params const& params, WorkTileInfo const& current_work) const {
    if constexpr (
        IsProducer && NumProducerThreads == cutlass::NumThreadsPerWarp) {
      // thread 0 already has the right tile_idx, just need to broadcast to the
      // rest of the producer threads (warp 0)
      return {__shfl_sync(0xffffffff, current_work.tile_idx, 0 /*lane*/)};
    } else if constexpr (
        IsProducer && NumProducerThreads == cutlass::NumThreadsPerWarpGroup) {
