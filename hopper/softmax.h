...
#pragma unroll
  ...
  __forceinline__ __device__ TensorT online_softmax(Tensor0& acc_s) {
    ...
    if constexpr (Is_first) {
      flash::template reduce_max</*zero_init=*/true>(scores, row_max);
      flash::template scale_apply_exp2<
          /*Scale_max=*/true,
          /*Check_inf=*/true,
          Use_max_offset>(scores, row_max, softmax_scale_log2);
      flash::reduce_sum</*zero_init=*/true, /*warp_reduce=*/false>(
          scores, row_sum);
      cute::fill(scores_scale, 1.f);
      ...
      // softmax.
      flash::reduce_sum</*zero_init=*/false, /*warp_reduce=*/false>(
          scores, row_sum);
    }
    return scores_scale;
  };
  ...
  finalize(Tensor0& acc_s, float descale_v = 1.f, float rp_dropout = 1.f) {
    constexpr static float max_offset_E =
        Use_max_offset ? 8.f * float(M_LN2) : 0.f;
    // Reshape acc_s from ((2, 2, V), MMA_M, MMA_N) to (nrow=(2, MMA_M),
    // ncol=(2, V, MMA_N))
    Tensor scores = make_tensor(
        acc_s.data(), flash::convert_layout_acc_rowcol(acc_s.layout()));
...
#pragma unroll
    for (int mi = 0; mi < size(row_max); ++mi) {
      ...
      row_sum(mi) = (sum == 0.f || sum != sum)
          ...
          : (row_max(mi) * softmax_scale_log2) * float(M_LN2) - max_offset_E +
              __logf(sum);
